"""
Distributed QAOA execution across multiple QPUs.
...
cudaq execution (qpp-cpu) or numpy simulation when hardware is unavailable.
"""
...
from src.qaoa_cold import run_qaoa, CUDAQ_AVAILABLE

if CUDAQ_AVAILABLE:
    import cudaq
    from src.qaoa_cold import kernel_qaoa, _sample_result_to_dict
...
def setup_mqpu(num_qpus: int | None = None) -> dict:
    ...
    Attempts cudaq targets in order:
      1. "nvidia-mqpu" (multi-GPU parallel sampling)
    ...
    if not CUDAQ_AVAILABLE:
        _BACKEND = "numpy"
        _NUM_QPUS = num_qpus or 4
        return {
            "backend": "numpy",
            "num_qpus": _NUM_QPUS,
            "note": "Numpy statevector fallback (cudaq unavailable). "
                    "Simulates distributed dispatch sequentially.",
        }
    ...
    return {
        "backend": "numpy",
        "num_qpus": _NUM_QPUS,
        "note": "Numpy statevector fallback.",
    }
...
def run_distributed_cold_qaoa(
    ...
    With qpp-cpu or numpy: sequential execution (identical math).
    """
    global _BACKEND, _NUM_QPUS
    if _BACKEND is None:
        setup_mqpu()
    ...
    if _BACKEND == "mqpu" and CUDAQ_AVAILABLE:
        futures = []
        for c, sp in enumerate(subproblems):
            qpu_id = c % _NUM_QPUS
